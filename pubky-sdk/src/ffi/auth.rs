...
use crate::ffi::types::{FfiAuthFlowInfo, FfiKeyProvider, FfiSessionDescriptor, FfiSignupOptions};
use crate::{AuthFlowKind, Capabilities, Pubky};
...
impl FfiSdk {
    ...
    pub fn new() -> Result<Arc<Self>, FfiPubkyError> {
        let pubky = Pubky::new().map_err(FfiPubkyError::from)?;
    ...
    pub async fn signin(
        ...
        let session = signer.signin().await.map_err(FfiPubkyError::from)?;
    ...
    pub async fn signup(
        ...
        let homeserver_pk =
            PublicKey::try_from(homeserver.as_str()).map_err(|e| FfiPubkyError::InvalidInput {
        ...
        let session = signer
            .signup(&homeserver_pk, signup_token_str.as_deref())
            .await
    ...
    ) -> Result<FfiAuthFlowInfo, FfiPubkyError> {
    ...
    pub async fn await_approval(
        ...
        let session = flow.await_approval().await.map_err(FfiPubkyError::from)?;
    ...
    pub async fn list_sessions(
        &self,
        key_provider: Arc<dyn FfiKeyProvider>,
    ) -> Result<Vec<FfiSessionDescriptor>, FfiPubkyError> {
        let signer = signer_from_provider(self.inner.as_ref(), key_provider)?;
        let sessions = signer.list_sessions().await.map_err(FfiPubkyError::from)?;
        Ok(sessions.iter().map(FfiSessionDescriptor::from).collect())
    }
...
fn signer_from_provider(
    sdk: &Pubky,
    key_provider: Arc<dyn FfiKeyProvider>,
) -> Result<crate::PubkySigner, FfiPubkyError> {
    let secret_key_bytes = key_provider.secret_key()?;
    if secret_key_bytes.len() != 32 {
        return Err(FfiPubkyError::InvalidInput {
            message: "Secret key must be 32 bytes".to_string(),

...
use crate::ffi::types::{FfiAuthFlowInfo, FfiKeyProvider, FfiSignupOptions};
...
impl FfiSdk {
    ...
    ///
    /// Does not copy host-local data from the previous homeserver.
    pub async fn migrate_homeserver(
        ...
        let signer = self.inner.signer(keypair);
        let session = signer
            .migrate_homeserver(&homeserver_pk, signup_token.as_deref())
            .await
            .map_err(FfiPubkyError::from)?;
